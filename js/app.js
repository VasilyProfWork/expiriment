 ...
 */    var MEDIA_PREFERS_REDUCED_MOTION = "(prefers-reduced-motion: reduce)";
    ...
    function forOwn(object, iteratee, right) {
        if (object) {
            ...
            for (var i = 0; i < keys.length; i++) {
                var key = keys[i];
                if ("__proto__" !== key) if (false === iteratee(object[key], key)) break;
            }
        }
        return object;
    }
    ...
    function Clones(Splide2, Components2, options) {
        ...
        function computeCloneCount() {
            ...
            if (!Splide2.is(LOOP)) clones2 = 0; else if (!clones2) {
    ...
    function Controller(Splide2, Components2, options) {
        ...
        function getAdjacent(prev, destination) {
            ...
            if (-1 === dest && isSlide) if (!approximatelyEqual(getPosition(), getLimit(!prev), 1)) return prev ? 0 : getEnd();
        ...
        function computeDestIndex(dest, from, snapPage) {
            if (isEnough()) {
                ...
                if (dest < 0 || dest > end) if (between(0, dest, from, true) || between(end, from, dest, true)) dest = toIndex(toPage(dest)); else if (isLoop) dest = snapPage ? dest < 0 ? -(slideCount % perPage || perPage) : slideCount : dest; else if (options.rewind) dest = dest < 0 ? end : 0; else dest = -1; else if (snapPage && dest !== from) dest = toIndex(toPage(from) + (dest < from ? -1 : 1));
            } else dest = -1;
            return dest;
        }
    ...
    function Arrows(Splide2, Components2, options) {
        ...
        var userArrows = Elements.arrows, track = Elements.track;
